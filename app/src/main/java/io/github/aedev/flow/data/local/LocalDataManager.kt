...
import io.github.aedev.flow.utils.CachedUpdateInfo
...
class LocalDataManager
    ...
    constructor(
        ...
        companion object {
            ...
            private val LAST_UPDATE_POLICY = stringPreferencesKey("last_update_policy")
            private val LAST_UPDATE_INFO = stringPreferencesKey("last_update_info")
        ...
        /**
         * Last policy this install successfully fetched. Cached so the mandatory-update gate can be
         * enforced on the very first frame of a cold start, before any network round trip, and so it
         * still applies when the device is offline.
         */
        val lastUpdatePolicy: Flow<UpdatePolicy?> =
            context.dataStore.data.map { prefs ->
                prefs[LAST_UPDATE_POLICY]?.let { UpdatePolicy.fromJson(it) }
            }

        suspend fun setLastUpdatePolicy(policy: UpdatePolicy) {
            context.dataStore.edit { prefs ->
                prefs[LAST_UPDATE_POLICY] = policy.toJson()
            }
        }

        /**
         * Last update this install saw available, still valid only for the build that found it.
         * Cached so the mandatory gate can open the installer from a cold start with no network
         * rather than letting an offline launch slip past it.
         */
        val lastUpdateInfo: Flow<CachedUpdateInfo?> =
            context.dataStore.data.map { prefs ->
                prefs[LAST_UPDATE_INFO]?.let { CachedUpdateInfo.fromJson(it) }
            }

        suspend fun setLastUpdateInfo(
            installedVersionName: String,
            info: UpdateInfo,
        ) {
            context.dataStore.edit { prefs ->
                prefs[LAST_UPDATE_INFO] = CachedUpdateInfo(installedVersionName, info).toJson()
            }
        }
        ...
        private fun deserializeCustomThemePalettes(
            raw: String?,
            legacyRaw: String?,
        ): CustomThemePalettes {
